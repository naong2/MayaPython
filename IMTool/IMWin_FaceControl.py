...
class DesignerUI(QtWidgets.QDialog):
    ...
    def connectSignals(self):
        """ signals for buttons """
        self.ui.clearFields_btm.clicked.connect(lambda: self.clearTextField())
        self.ui.centerLoc_btm.clicked.connect(lambda: self.loadInTextField(True))
        self.ui.upVecLoc_btm.clicked.connect(lambda: self.loadInTextField(False))
        self.ui.nurbsSurface_btm.clicked.connect(lambda: self.loadInTextField(False))
        self.ui.uPatch_hs.valueChanged.connect(lambda: self.displayLCD())
        self.ui.vPatch_hs.valueChanged.connect(lambda: self.displayLCD())
        self.ui.createNurbsSurface_btm.clicked.connect(lambda: self.createNurbsAttachSurface())
        self.ui.createLoc_btm.clicked.connect(lambda: self.createLocatorsUpCenter())
        self.ui.createCtrl_btm.clicked.connect(lambda: self.runControlCreator())
        self.ui.setup_btm.clicked.connect(lambda: self.setup())
        self.ui.mesh_btm.clicked.connect(lambda: self.populateTextField("mesh"))
        self.ui.ctrl_btm.clicked.connect(lambda: self.populateTextField("ctrl"))
        self.ui.faces_btm.clicked.connect(lambda: self.populateTextField("faces", True))

    def displayLCD(self):
        self.ui.uPatch_lcd.display(str(self.ui.uPatch_hs.value()))
        self.ui.vPatch_lcd.display(str(self.ui.vPatch_hs.value()))
    ...
    def second(self, upVector='', fromCenter=False, objects=[]):
        ...
        for s in sel:
            ...
            if fromCenter:
                ...
                cmds.aimConstraint(loc, par, mo=1, weight=1, aimVector=(1, 0, 0), upVector=(0, 1, 0),
                                   worldUpType="object", worldUpObject=upVector)
            allLocs.append(loc)
        return allLocs
    ...
    def attachToNurbsSurface(self, prefix='', nurbSurface='', surfaceVec=[]):
        ...
        for i in range(len(surfaceVec)):
            ...
            closestPointOnSurface = cmds.createNode('closestPointOnSurface',
                                                    n=surfaceVec[i] + '_closestPointOnSurface')
            ...
            cmds.connectAttr(closestPointOnSurface + ".result.parameterV", pointOnSurfaceInfo + ".parameterV",
                             f=True)
            cmds.connectAttr(closestPointOnSurface + ".result.parameterU", pointOnSurfaceInfo + ".parameterU",
                             f=True)
            # finding the cross product for XYZ
        ...
        if (len(sel) == 1):
            if (cmds.listRelatives(sel[0], s=True)):
                ...
                if 'nurbsSurface' in types:
                    ...
                    self.ui.nurbsSurface_le.setText(sel[0])
                    ...
                    self.ui.centerLoc_le.setText(sel[0])
                    ...
                    self.ui.upVecLoc_le.setText(sel[0])
    ...
    def createNurbsAttachSurface(self, *args):
        ...
        prefix = str(self.ui.prefix_le.text())
        uPatches = self.ui.uPatch_hs.value()
        vPatches = self.ui.vPatch_hs.value()
        nurbsObject = cmds.nurbsPlane(p=[0, 0, 0], ax=[0, 1, 0], w=1, lr=1, d=3, u=uPatches, v=vPatches, ch=1,
                                      n=prefix + '_attachSurface#')
        self.ui.nurbsSurface_le.setText(nurbsObject[0])
        print('complete', 'Slide Nurbs Surface loaded.')
    ...
    def createLocatorsUpCenter(self, *args):
        ...
        self.ui.centerLoc_le.setText(centerLocator[0])
        self.ui.upVecLoc_le.setText(upVecLocator[0])
        print('complete', 'Locators Loaded.')

    def clearTextField(self, *args):
        """ clears text fields in GUI """
        self.ui.nurbsSurface_le.setText("")
        self.ui.upVecLoc_le.setText("")
        self.ui.centerLoc_le.setText("")
    ...
    def createLowRezCurve(self, prefix, hiRezCurve):
        ...
        tempCrv = cmds.rebuildCurve(hiRezCurve, ch=1, rpo=0, rt=0, end=1, kr=0, kcp=0, kep=1, kt=0, s=2, d=3,
                                    tol=0.01)
    ...
    def runControlCreator(self, *args):
        ...
        controlScale = self.ui.ctrlSize_dpb.value()
        upVecForCenter = str(self.ui.upVecLoc_le.text())
        centerLocator = str(self.ui.centerLoc_le.text())
        nurbSurface = str(self.ui.nurbsSurface_le.text())
        ...
        limitHiRexJoints = self.ui.jointLimiting_sb.value()
        tmpJNTs = []
        ...
        cmds.wire(hiRezCurve, w=lowRezCurve, gw=False, en=1.000000, ce=0.000000, li=0.000000, dds=[(0, 100)],
                  n=prefix + '_lowRez_wire')
        ...
        if findYourCenter:
            ...
            scaleGrp = cmds.group(hierarchyGrp[0], hierarchyGrp[1], hierarchyGrp[2], upVecForCenter, centerLocator,
                                  n=prefix + '_scale_grp')
            nonDeformableGrp = cmds.group(hiRezCurveGrp, lowRezCurveGrp, hierarchyGrp[3],
                                          n=prefix + '_nonDeform_grp')
        ...
        # parent under center locator?
            ...
            nonDeformableGrp = cmds.group(hiRezCurveGrp, lowRezCurveGrp, hierarchyGrp[2],
                                          n=prefix + '_nonDeform_grp')
    ...
    def transformGeometrySetup(self):
        ...
        if self.ui.includeMeshTrans_cb.isChecked():
    ...
    def setup(self, *args):
        ...
        self.meshes = Meshes()
        faceLineEdit = str(self.ui.faces_le.text())
        faceSelection = faceLineEdit.split(",")
        self.deformingShape = ''

        self.ctrl.controls = pm.PyNode(str(self.ui.ctrl_le.text()))
        self.meshes.source = pm.PyNode(str(self.ui.mech_le.text()))
    ...
    def updateCtrlShape(self, controller, targetMesh):
        if not self.ui.retainOG_cb.isChecked():
        ...
        if len(sel):
            ...
            if faces:
                ...
                for s in sel:
                    if ".f" in str(s):
                        if s == sel[-1]:
                            faceStrings += "{0}".format(str(s))
                        else:
                            faceStrings += "{0},".format(str(s))
                    else:
                        pm.error("Please select faces!")
                        continue
                self.ui.faces_le.setText(str(faceStrings))
            # pm.textFieldButtonGrp( control,e=1,text=str(faceStrings))
            else:
                if control == "mesh":
                    self.ui.mech_le.setText(str(sel[0]))
                else:
                    self.ui.ctrl_le.setText(str(sel[0]))
        else:
...
def main():
    ...
    uifile = os.path.join(imu.getPathIMTool(),'IMWin_FaceControl_wd.ui').replace('\\','/')
    template_dialog = DesignerUI(ui_path=uifile, title_name='Face Control')
